@php
    /** @var \App\Entity\Jira\Operator $operator */
    /** @var \Illuminate\Pagination\LengthAwarePaginator $operators */
@endphp
...
@section('content')
    ...
    <table class="table table-striped table-messages">
        ...
        @forelse($operators as $operator)
            <tr class="d-flex">
                <td class="col-1">
                    <img style="width: 30px;" src="{{ $operator->rCreator->avatar ?? '' }}">
                </td>
                <td class="col-3">{{ $operator->user_key ?? '' }}</td>
                <td class="col-3">{{ $operator->rCreator->display_name ?? '' }}</td>
                <td class="col-1">{{ $operator->rLine->name ?? '' }}</td>
            </tr>
        @empty
            <p>No creators</p>
        @endforelse
